    ...
    public void save(YPRestaurantMenu restaurantMenu)  {
    	...
    	for(YPCategory category : restaurantMenu.getCategories()) {
    		for(YPDish dish : category.getDishes()) {
    			...
    			mdishes.add(new BasicDBObject("name", dish.getName()).append("description", dish.getDescription()).append("price", dish.getPrice()).append("categories", categoriesTag));
    		}
    	}
    	
    	mmenu.append("mid", restaurantMenu.getId());
    	mmenu.append("name", restaurantMenu.getName());
    	mmenu.append("address", new BasicDBObject("street", restaurantMenu.getStreet()).append("city", restaurantMenu.getCity()).append("state", restaurantMenu.getState()).append("zip", restaurantMenu.getZip()));
